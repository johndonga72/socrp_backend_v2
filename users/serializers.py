...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = CustomUser
        fields = ["id", "email", "full_name"]  # add more fields if needed

# --- Education Serializer ---
class EducationSerializer(serializers.ModelSerializer):
    id = serializers.IntegerField(required=False)  # include ID for update
    class Meta:
        model = Education
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if isinstance(experiences_data, str):
            experiences_data = json.loads(experiences_data)
    ...
    def update(self, instance, validated_data):
        ...
        existing_edu_ids = [edu.id for edu in instance.educations.all()]
        sent_edu_ids = []
        for edu in educations_data:
            edu_id = edu.get("id")
            if edu_id and edu_id in existing_edu_ids:
                # Update existing
                Education.objects.filter(id=edu_id, user_profile=instance).update(**edu)
                sent_edu_ids.append(edu_id)
            else:
                # Create new
                new_edu = Education.objects.create(user_profile=instance, **edu)
                sent_edu_ids.append(new_edu.id)

        # Delete removed educations
        Education.objects.filter(user_profile=instance).exclude(id__in=sent_edu_ids).delete()

        # Update or create experiences
        existing_exp_ids = [exp.id for exp in instance.experiences.all()]
        sent_exp_ids = []
        for exp in experiences_data:
            exp_id = exp.get("id")
            if exp_id and exp_id in existing_exp_ids:
                WorkExperience.objects.filter(id=exp_id, user_profile=instance).update(**exp)
                sent_exp_ids.append(exp_id)
            else:
                new_exp = WorkExperience.objects.create(user_profile=instance, **exp)
                sent_exp_ids.append(new_exp.id)

        # Delete removed experiences
        WorkExperience.objects.filter(user_profile=instance).exclude(id__in=sent_exp_ids).delete()
